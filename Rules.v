...
(*------------ wordcount opertions -------------------*)
(* mapper *)
Lemma hoare_wdmap : forall Hf Hb bp ll,
  hoare (bval_wdmap (val_bloc bp))
    (\R[ Hf, ( (bp ~b~> ll) \b* Hb) ])
    ( fun r => \[r = (val_listwdpair (wordmapper ll))] \* \R[ Hf, ((bp ~b~> ll) \b* Hb)] ).
Proof.
  intros. intros sf sb K0.
  destruct K0 as (P1&P2).
  destruct P2 as (sb1&sb2&D1&D2&D3&D4).
  exists sf sb (val_listwdpair (wordmapper ll)). subst.
  splits~.
...
(* merge *)
Lemma hoare_wdmerge : forall H L,
  hoare (fval_wdmerge (val_Listwd L)) H
    (fun r => (\[r = val_listwdpair (wordmerge L)] \* H)).
Proof.
  intros. intros sf sb.
  intros M.
  exists sf sb (val_listwdpair (wordmerge L)). splits.
  apply eval_wdmerge.
  applys* hstar_hpure_iff.
Qed.

(* shuffle *)
Lemma hoare_wdshuffle : forall H l,
  hoare (fval_wdshuffle (val_listwdpair l)) H
    (fun r => (\[r = val_Listwd (wordshuffle l)] \* H)).
Proof.
  intros. intros sf sb.
  intros M.
  exists sf sb (val_Listwd (wordshuffle l)). splits.
  apply eval_wdshuffle.
  applys* hstar_hpure_iff.
Qed.

(* reduce *)
Lemma hoare_wdreduce : forall H L,
  hoare (fval_wdreduce (val_Listwd L)) H
    (fun r => (\[r = val_listwdpair (wordreducer L)] \* H)).
Proof.
  intros. intros sf sb.
  intros M.
  exists sf sb (val_listwdpair (wordreducer L)). splits.
  apply eval_wdreduce.
  applys* hstar_hpure_iff.
Qed.


(*------------ invertindex opertions -------------------*)
(* locate word *)
Lemma hoare_locate : forall Hf Hb bp ll,
  hoare (bval_locate (val_bloc bp))
    (\R[ Hf, ( (bp ~b~> ll) \b* Hb) ])
    ( fun r => \[r = (val_listindex (locate ll bp))] \* \R[ Hf, ((bp ~b~> ll) \b* Hb)] ).
Proof.
  intros. intros sf sb K0.
  destruct K0 as (P1&P2).
  destruct P2 as (sb1&sb2&D1&D2&D3&D4).
  exists sf sb (val_listindex (locate ll bp)). subst.
  splits~.
  - lets (E1&N) : hbsingle_inv D1. subst. simpls.
    applys~ eval_locate_sep. apply D4.
  - rewrite~ hstar_hpure. splits~. splits~. simpls. substs~.
    applys~ hbstar_intro.
Qed.

(* mapper *)
Lemma hoare_iimap : forall H l,
  hoare (bval_iimap (val_listindex l)) H
    (fun r => (\[r = val_listiipair (iimapper l)] \* H)).
Proof.
  intros. intros sf sb.
  intros M.
  exists sf sb (val_listiipair (iimapper l)). splits.
  apply eval_iimap.
  applys* hstar_hpure_iff.
Qed.

(* merge *)
Lemma hoare_iimerge : forall H L,
  hoare (fval_iimerge (val_Listii L)) H
    (fun r => (\[r = val_listiipair (iimerge L)] \* H)).
Proof.
  intros. intros sf sb.
  intros M.
  exists sf sb (val_listiipair (iimerge L)). splits.
  apply eval_iimerge.
  applys* hstar_hpure_iff.
Qed.

(* shuffle *)
Lemma hoare_iishuffle : forall H l,
  hoare (fval_iishuffle (val_listiipair l)) H
    (fun r => (\[r = val_Listii (iishuffle l)] \* H)).
Proof.
  intros. intros sf sb.
  intros M.
  exists sf sb (val_Listii (iishuffle l)). splits.
  apply eval_iishuffle.
  applys* hstar_hpure_iff.
Qed.

(* reduce *)
Lemma hoare_iireduce : forall H L,
  hoare (fval_iireduce (val_Listii L)) H
    (fun r => (\[r = val_listiipair (iireducer L)] \* H)).
Proof.
  intros. intros sf sb.
  intros M.
  exists sf sb (val_listiipair (iireducer L)). splits.
  apply eval_iireduce.
  applys* hstar_hpure_iff.
Qed.
...
(** WordCount rules **)
Lemma triple_wdmap : forall Hf bp ll,
  triple (bval_wdmap (val_bloc bp))
    (\R[ Hf, (bp ~b~> ll) ])
    ( fun r => \[r = (val_listwdpair (wordmapper ll))] \* \R[ Hf, (bp ~b~> ll)] ).
Proof.
...
Lemma triple_wdmerge : forall H L,
  triple (fval_wdmerge (val_Listwd L)) H
    (fun r => (\[r = val_listwdpair (wordmerge L)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_listwdpair (wordmerge L)). splits.
  apply eval_wdmerge. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

Lemma triple_wdshuffle : forall H l,
  triple (fval_wdshuffle (val_listwdpair l)) H
    (fun r => (\[r = val_Listwd (wordshuffle l)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_Listwd (wordshuffle l)). splits.
  apply eval_wdshuffle. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

Lemma triple_wdreduce : forall H L,
  triple (fval_wdreduce (val_Listwd L)) H
    (fun r => (\[r = val_listwdpair (wordreducer L)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_listwdpair (wordreducer L)). splits.
...
(** IndexInvert rules **)
Lemma triple_locate : forall Hf bp ll,
  triple (bval_locate (val_bloc bp))
    (\R[ Hf, (bp ~b~> ll) ])
    ( fun r => \[r = (val_listindex (locate ll bp))] \* \R[ Hf, (bp ~b~> ll)] ).
Proof.
  intros. unfold triple. intros Hf' Hb'.
  applys hoare_conseq hoare_locate.
  rewrite hstar_sep. apply himpl_refl.
  intros r h R.
  rewrite hstar_hpure_iff in R.
  destruct R as (RA&RB).
...
Lemma triple_iimap : forall H ll,
  triple (bval_iimap (val_listindex ll)) H
    ( fun r => \[r = (val_listiipair (iimapper ll))] \* H ).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_listiipair (iimapper ll)). splits.
  apply eval_iimap. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

Lemma triple_iimerge : forall H L,
  triple (fval_iimerge (val_Listii L)) H
    (fun r => (\[r = val_listiipair (iimerge L)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_listiipair (iimerge L)). splits.
  apply eval_iimerge. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

Lemma triple_iishuffle : forall H l,
  triple (fval_iishuffle (val_listiipair l)) H
    (fun r => (\[r = val_Listii (iishuffle l)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_Listii (iishuffle l)). splits.
  apply eval_iishuffle. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

Lemma triple_iireduce : forall H L,
  triple (fval_iireduce (val_Listii L)) H
    (fun r => (\[r = val_listiipair (iireducer L)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_listiipair (iireducer L)). splits.
  apply eval_iireduce. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

Lemma triple_iiorganize : forall H l,
  triple (fval_iiorganize (val_listiipair l)) H
    (fun r => (\[r = val_listenv (iiorganize l)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_listenv (iiorganize l)). splits.
  apply eval_iiorganize. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

(* rules for aux list operations *)
Lemma triple_app_wdlist : forall H l L,
  triple (val_app_wdlist (val_listwdpair l) (val_Listwd L)) H
    (fun r => (\[r = val_Listwd (l :: L)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_Listwd (l :: L)). splits.
  apply eval_app_wdlist. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

Lemma triple_app_idxlist : forall H l L,
  triple (val_app_idxlist (val_listindex l) (val_Listidx L)) H
    (fun r => (\[r = val_Listidx (l :: L)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_Listidx (l :: L)). splits.
  apply eval_app_idxlist. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.

Lemma triple_app_iilist : forall H l L,
  triple (val_app_iilist (val_listiipair l) (val_Listii L)) H
    (fun r => (\[r = val_Listii (l :: L)] \* H)).
Proof.
  intros. intros Hf Hb sf sb M.
  exists sf sb (val_Listii (l :: L)). splits.
  apply eval_app_iilist. rewrite hstar_assoc.
  applys* hstar_hpure_iff.
Qed.
...
(* ================================================================================ *)
    ...
    (** ____________________________Previous Rules___________________________ **)
...
Lemma triple_MRlist_reform : forall H l1,
  triple (val_reform (val_listwdpair l1)) H
    (fun r => (\[r = val_listint (filetrans l1)] \* H)).
Proof.
  intros. unfold triple. intros Hf Hb sf sb M.
  exists sf sb (val_listint (filetrans l1)).
